...
Disposable income after total spending: {disposable}
...
"""
    try:
        ...
        narrative = (
            f"Based on your income of {CURRENCY} {profile['income']:,.2f} and total logged "
            f"spending of {CURRENCY} {summary['total_spent']:,.2f}, we recommend saving about "
...
food budget, family size, dietary needs), their logged expenses (each tagged
as either 'food' or 'custom' for non-food household spending), a computed
budget summary (spend by category, planned vs actual, flagged overspending
categories, unusual/outlier expenses, and a food-vs-custom spending split),
and locally sourced grocery data (food_prices, recipes, recipe_ingredients,
ingredient_prices) pulled from a real SQLite database. Use ONLY the provided
data as your source of truth — do not invent products, prices, or figures
not present in the context. Always report costs in {CURRENCY}. Give
practical, budget-conscious, nutritionally sensible suggestions. Show simple
math (price per serving, weekly/monthly totals) when relevant. Reference the
user's actual flagged categories or unusual expenses when relevant to their
question. Keep answers concise and use markdown formatting (bullet points,
bold, small tables) for readability. If the data provided is insufficient to
answer precisely, say so honestly rather than guessing."""
